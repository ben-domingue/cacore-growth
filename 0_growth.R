...
growth<-function(df,reliability=0.9,g11,
                 sampling=NULL,
                 fm.edit=NULL) {
    library(mecor)
    ##sampling
...
########################################
    ...
    if (!is.null(fm.edit)) {## this will update formula for subgroup analyses
        test<-fm.base %in% fm.edit
        fm.base<-fm.base[!test]
        print(fm.base)
    }
    ##
    fm.list<-character()
...
########################################
    ...
    ##fm0<-gsub("MeasErrorRandom(lag_scale_score,var(lag_scale_score,na.rm=TRUE)*(1-reliability))","lag_scale_score",fm,fixed=TRUE) ##for checking with lm()
...
########################################
    ...
    ##note that this works for school-grade estimates; school estimates are handled separately in 02b_
    S<-summary(m3)$coef
    ...
    co$n<-unlist(out)
    co$fe<-co$fe-mean(co$fe,na.rm=TRUE) ##unweighted
